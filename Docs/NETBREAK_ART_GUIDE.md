...
- **미정:** 실제 픽셀·카메라·애니메이션·팔레트 제작 규격.
- **검증 필요:** 정어리 외 에셋의 Unity 화면 적용, 전체 성능, 폰트와 외부 에셋 라이선스. 정어리 첫 프로토타입의 Unity 수동 시각 검증은 완료했다.
- **미완료:** 정어리 첫 프로토타입 외 정식 에셋 제작·적용, 정식 VFX·SFX·BGM 및 최종 출시용 Art Lock.
...
이는 제작 방향이며 기존 Sprite를 확인하거나 교체 완료했다는 뜻이 아니다. 대규모 어군에서도 개체와 역할을 읽을 수 있어야 한다.
...
32px, 48px, 64px는 비교 후보일 뿐 확정값이 아니다. 콘셉트 이미지의 밀도만으로 실제 규격을 고정하지 않는다.
...
적용 뒤 실제 카메라에서의 크기, 밀집 실루엣, 배경 대비, 도구와 물고기 구분, 픽셀 크기 일관성, 기존 VFX와의 조화, 주요 해상도 가독성과 다수 물고기 성능을 확인한다. 정어리 한 종의 첫 프로토타입은 VS-2B-1에서 제작·적용하고 수동 시각 검증을 마쳤다. 다른 후보 에셋과 전체 화면·성능 검증은 후속 작업이다.
