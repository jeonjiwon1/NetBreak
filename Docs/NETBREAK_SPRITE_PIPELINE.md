...
VS-2B-4 시점에는 복어 팽창·가시 강화·특수 상태 전용 애니메이션과 오징어 먹물 발사 전용 프레임·애니메이션이 없었다. VS-2C-1에서 오징어 먹물 공격은 별도 64×64 셀·4방향축×4프레임 시트로 추가했다. `NETBREAK/Art/Setup Squid Ink Presentation`이 두 신규 시트를 분할하고 `SquidInkPresentation` Profile에 16개 공격 Sprite·4개 먹물 구름 Sprite·WAV를 연결한다. `NETBREAK/Art/Validate Squid Ink Presentation`은 연결과 Import 설정을 확인한다. 기존 수영 16프레임 시트와 프로필은 보존한다. Unity 배치 메뉴 실행·Validate·전체 EditMode 190/190 통과와 사용자 Play Mode 수동 검증을 마쳤고 현재 Prototype 품질을 승인했다. 복어 특수 연출과 최종 VFX/SFX·최종 아트 승인은 후속 대상이다.

VS-2C-2에서 같은 두 메뉴는 16×16 Ink Projectile 4프레임과 24×24 Ink Impact 4프레임의 가져오기·Profile 연결·검사까지 확장됐다. 사용자가 Unity compile 정상·Console Error 0·전체 EditMode 193/193 통과와 Play Mode에서 두 새 시트의 표시 및 Release → Projectile → Impact → 지속 방해 상태 흐름을 확인했다. 두 VFX의 Manual Visual Validation은 Passed, Prototype Approval은 Approved이며 Final Production VFX Approval은 Pending이다. 메뉴 실행 여부는 별도로 전달받지 않았다. 기존 VS-2C-1의 190/190 기록은 이 확장 이전 결과다.
